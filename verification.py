...
def main():
    ...
    AQUISITION_TIME = int(0.05*60E9) # in picosecond
    N_REP = 1
    ...
    #phase=-69.009982
     ...
     #tt.real_time_measure(AQUISITION_TIME,N_REP, GROUPS, COINCIDENCE_WINDOW, count_singles=False, data_filename=f"\ABCD={label}_{playerlabels}.txt", save_raw=True, save_params=True)    
    ...
    for z in range(3):
        ...
        for j in range(len(Bases)):
            ...
            print("Gathering the counts for bad (no bad words) analysis")
            tt.real_time_measure(AQUISITION_TIME,N_REP, GROUPS, COINCIDENCE_WINDOW, count_singles=False, data_filename=f"\{label}_{Bit_label}_{z}.txt", save_raw=True, save_params=True)
